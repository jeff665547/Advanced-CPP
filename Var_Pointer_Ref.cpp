...
// 6. const 在當型別使用時會先形容其左邊的型態，若左邊沒有東西才會去形容右邊的型態，
//    如：const T* ptr 與 T const *ptr的意思相同，const都是形容T，意思都是一個名字
//    叫做ptr的指標去指向const T物件，也就是說ptr指向的該物件其內容是唯讀的，無法被更動，
//    但是ptr指標本身是可以去指向另一個不能被更動的const T物件。
...
// 7. const T* ptr的意思是有一個名稱(字)叫ptr(向右沒有東西，再向左碰到*代表指標)的指標(再向左，碰到const T)，
//    指向const T，因此ptr指向的T物件不能被更改，但是ptr本身可以指向另一個不能被更動的T物件，因此更改ptr
//    儲存的位址是可以的(i.e. 但*ptr = t是不行的，但ptr++是可以的)。
// 8. T* const ptr的意思是有一個名稱(字)叫ptr(向右沒有東西，再向左碰到* const代表const指標)的const指標，
